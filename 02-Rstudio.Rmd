...
[Link to site](https://blog.rstudio.com/2020/02/18/rstudio-1-3-preview-configuration/).  
...
## Script to copy the packages installed on one computer or R version to another. (!THE ONLY METHOD THAT WORKED) USE THIS ONE!!!!!].  
...
## Quick Way of Installing all your old R libraries on a New Device. DOES NOT WORK!!    
...
1. Other problem is the code chunks.  **_Always use the `{r echo=TRUE, eval=FALSE}` at the beginning of the chunk code_**  
...
To have a better control on how to display the code chunks and plots. In the chunk Preferences. 
...
[source01](https://stackoverflow.com/questions/25166624/insert-picture-table-in-r-markdown).  
...
Install the BetterBibTeX plugin in Zotero and then use the **citr** package in R. See here for General instructions on adding citations to an R Markdown document (though citr makes this easier):   
...
_Note: the default setting of BBT will generate different citekeys than Zotero would itself generate; the keys from Zotero are not always safe for use in bibtex/biber. If you want to get the stock zotero keys, set the pattern in the preferences to [zotero]. I very much recommend **not** choosing [zotero] as your pattern unless you have existing articles that use keys generated by previous Zotero-native BibTeX export._  
...
Whach this [video](https://youtu.be/GAVXVkcpbG0) as guide:   
...
###Book down and GitHub Pages. 
Source: https://youtu.be/RdSmYvbQkhs 

1.	Create bookdow from template.   All the files needed for bookdow are automatically created. 
2.	To Upload to GitHub go to Terminal in Rstudio.
...
git add .
git commit -m”Add text file in case”
...
Hosting bookdown in github - R Markdown - RStudio Community
...
Erase portions in the .yml files and comit through R.
Edit setting in github brawser to show the webpage from “docs” folder.
